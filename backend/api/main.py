...
def dashboard_stats(db: Session = Depends(get_db)):
    ...
    needs_review = db.query(Document).filter(
        Document.status == DocumentStatus.REVIEW_NEEDED
    ).count()
    ...
    open_findings = db.query(DBFinding).filter(DBFinding.status == "open").count()
    return {
        ...
        "total": processing + needs_review + completed + failed
    }
...
def list_documents(db: Session = Depends(get_db)):
    ...
    # Get findings counts per document in one query
    findings_counts = dict(
        db.query(DBFinding.document_id, func.count(DBFinding.id))
        .filter(DBFinding.status == "open")
        .group_by(DBFinding.document_id)
        .all()
    )
    ...
    return {"documents": [
        {
            ...
            "filename": d.filename,
            "status": d.status,
            "doc_type": (d.extraction_result or {}).get("doc_type"),
            "created_at": d.created_at,
            "open_findings": findings_counts.get(d.id, 0)
        } for d in docs
    ]}

@app.get("/documents/{doc_id}/pdf")
def get_document_pdf_url(doc_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Could not generate PDF URL")
...
def assess_risk(doc_id: int, db: Session = Depends(get_db)):
    """Enqueue risk assessment as a background task. Returns task_id for polling."""
    doc = db.query(Document).filter(Document.id == doc_id).first()
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")

    task = celery_app.send_task("assess_risk", args=[doc_id])
    return {"status": "queued", "task_id": task.id, "document_id": doc_id}
...
def review_finding(
    ...
    # If all findings for this document are resolved, mark document as COMPLETED
    open_count = db.query(DBFinding).filter(
        DBFinding.document_id == finding.document_id,
        DBFinding.status == "open"
    ).count()
    if open_count == 0:
        doc = db.query(Document).filter(Document.id == finding.document_id).first()
        if doc and doc.status == DocumentStatus.REVIEW_NEEDED:
